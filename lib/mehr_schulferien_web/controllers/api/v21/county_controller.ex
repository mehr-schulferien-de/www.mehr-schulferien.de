defmodule MehrSchulferienWeb.Api.V21.CountyController do
  @moduledoc """
  API v2.1 controller for counties.

  Provides endpoints for:
  - Listing all counties
  - Getting a specific county by slug
  - Getting periods for a county
  - Generating iCalendar for a county
  """
  ...
  def index(conn, params) do
    ...
    render_json(conn, counties)
  end

  def show(conn, %{"slug" => slug}) do
    with {:ok, county} <- get_county_by_slug(slug) do
      render_json(conn, format_location(county))
    end
  end
  ...
  def periods(conn, %{"slug" => slug} = params) do
    with {:ok, county} <- get_county_by_slug(slug) do
      ...
      render_json(conn, periods, %{
        location: format_location(county),
        date_range: %{
  ...
  def icalendar(conn, %{"slug" => slug} = params) do
    with {:ok, county} <- get_county_by_slug(slug) do
      ...
      ical_content = CustomICal.generate(periods, county)

      conn
      |> put_resp_header("content-type", "text/calendar; charset=utf-8")
      |> put_resp_header(
        "content-disposition",
        build_ical_filename(county.name, ical_params.year, is_school_year)
      )
      |> text(ical_content)
    end
  end
  ...
  def bridge_days(conn, %{"slug" => slug} = params) do
    with {:ok, county} <- get_county_by_slug(slug),
         {:ok, year} <- parse_year(params["year"]),
         true <- BridgeDayController.has_bridge_days?([county.id], year) do
      result = BridgeDayFormatter.format_bridge_days(county, year)
  ...
  defp get_county_by_slug(slug) do
    query =
      from l in Location,

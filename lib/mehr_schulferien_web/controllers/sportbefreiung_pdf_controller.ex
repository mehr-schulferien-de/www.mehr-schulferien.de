defmodule MehrSchulferienWeb.SportbefreiungPdfController do
  ...
  def download(conn, %{"school_slug" => school_slug} = params) do
    try do
      ...
      case PdfGenerator.generate_sportbefreiung_pdf(form_data, school) do
        {:ok, pdf_binary} ->
        ...
        {:error, reason} ->
          ...
          |> redirect(to: "/briefe/#{school_slug}/sportbefreiung")
      end
    rescue
      Ecto.NoResultsError ->
        conn
  ...
  defp extract_form_data(params) do
    ...
    %{
      ...
      greeting: greeting,
      duration_type: params["duration_type"] || "single_lesson",
      single_date: parse_date(params["single_date"]),
      start_date: parse_date(params["start_date"]),
      end_date: parse_date(params["end_date"]),
      child_type: params["child_type"] || "mein_sohn",
      detailed_reason: params["detailed_reason"] || "",
      medical_certificate: params["medical_certificate"] == "true"
    }
  end
  ...
  defp generate_filename(form_data) do
    ...
    date_str =
      if form_data.duration_type == "single_lesson" do
        Date.to_iso8601(form_data.single_date)
      else
    ...
    "Sportbefreiung_#{student_name}_#{date_str}.pdf"
  end
end

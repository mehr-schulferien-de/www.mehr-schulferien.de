defmodule MehrSchulferien.GoogleSearchAPI do
  @moduledoc """
  Module for interacting with the Google Search API via SerpApi.
  
  This module provides functionality to search for schools using Google Search results
  to enrich or verify school homepage URLs.
  
  ## Configuration
  
  This module requires the `GOOGLE_SEARCH_API_KEY` environment variable to be set
  ...
  ## Examples
      ...
      iex> GoogleSearchAPI.search_school("Görres Gymnasium", "Koblenz, Rhineland-Palatinate, Germany")
      {:ok, %{"organic_results" => [...], ...}}
  ...
  def search_school_by_slug(school_slug, opts \\ []) do
    ...
    try do
      ...
      search_results = if school.address && school.address.google_search_cache && !force_refresh do
        # Use cached results
  ...
  def find_most_likely_homepage(search_results, school_name) do
    ...
    scored_urls = Enum.map(urls, fn url_info ->
    ...
    sorted_urls = Enum.sort_by(scored_urls, fn url ->
  ...
  defp calculate_homepage_score(url_info, normalized_school_name) do
    ...
    score = if String.contains?(String.downcase(url_info.title || ""), normalized_school_name) do
      score + 10
    else
      score
    end
    ...
    school_patterns = [".schule.", ".gymnasium.", ".realschule.", ".gesamtschule.", 
                      ".grundschule.", ".hauptschule.", ".mittelschule."]
    
    score = if Enum.any?(school_patterns, &String.contains?(link, &1)) do
      score + 5
    else
      score
    end
    ...
    score = if url_info.position == 1 do
      score + 3
    else
      score
    end
    ...
    excluded_domains = ["facebook.com", "instagram.com", "twitter.com", "wikipedia.org", 
                       "schulen.de", "schule-studium.de", "meinestadt.de", "youtube.com",
                       "mehr-schulferien.de"]
    
    score = if Enum.any?(excluded_domains, &String.contains?(link, &1)) do
      score - 10
    else
      score
    end
    ...
    score = if String.contains?(link, ".de/") and not String.contains?(link, "aalen.de") do
      score + 2
    else
      score
    end
  ...
  def extract_school_info(search_results, school_name) do
    ...
    info = if kg = search_results["knowledge_graph"] do
      info
    ...
    info = case find_most_likely_homepage(search_results, school_name) do
  ...
  defp extract_from_knowledge_graph(info, kg) do
    ...
    |> Map.put(:description, summarize_description(kg["description"]))
    |> Map.put(:additional_info, Map.merge(info.additional_info, %{
      website_from_kg: kg["website"],
      address: kg["address"],
      students_count: kg["schüler"],
      founded: kg["gründung"],
  ...
  defp maybe_extract_phone_from_snippet(info, snippet) do
    ...
    if is_nil(info.phone_number) do
      ...
      case Regex.run(~r/(?:Tel\.?|Telefon|Fon|☎️?):?\s*([+]?49[\s.-]?[\d\s.-]+|\(?\d{3,5}\)?[\s.-]?\d{3,10})/, snippet) do
  ...
  defp maybe_extract_description_from_snippet(info, snippet, link) do
    ...
    if is_nil(info.description) and 
       (String.contains?(link, ".schule.") or 
        String.contains?(link, ".gymnasium.") or
        String.contains?(link, "wikipedia.org")) do
  ...
  def summarize_description(description) when is_binary(description) do
    ...
    if ollama_available?() do
      ...
      # If Ollama is not available, return original description
      description
    end
  end

defmodule Mix.Tasks.SearchSchool do
  ...
  alias MehrSchulferien.GoogleSearchAPI
  ...
  defp process_school_search(school_slug, opts) do
    ...
    # Build API options
    api_opts = if opts[:force_refresh] do
    ...
    case GoogleSearchAPI.search_school_by_slug(school_slug, api_opts) do
  ...
  defp display_school_info(school, locations) do
    ...
    if school.address do
      ...
      if school.address.homepage_url, do: Mix.shell().info("  Current Homepage: #{school.address.homepage_url}")
      if school.address.email_address, do: Mix.shell().info("  Email: #{school.address.email_address}")
      if school.address.phone_number, do: Mix.shell().info("  Phone: #{school.address.phone_number}")
    end
  ...
  defp display_extracted_urls(results) do
    ...
    urls = GoogleSearchAPI.extract_homepage_urls(results)
  ...
  defp display_homepage_finding(results, school_name) do
    ...
    case GoogleSearchAPI.find_most_likely_homepage(results, school_name) do
      {:ok, homepage_url} ->
  ...
  defp display_comprehensive_info(results, school_name) do
    ...
    info = GoogleSearchAPI.extract_school_info(results, school_name)

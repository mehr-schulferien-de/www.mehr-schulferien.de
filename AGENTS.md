...
- Seasonal year pages (for example `/weihnachtsferien/2027`) must select periods by their start year and retain the full end date, even in the next year. Do not reuse overlap-only year filtering for these pages; test prior-year spillover, next-year endings, summary totals and JSON-LD together.

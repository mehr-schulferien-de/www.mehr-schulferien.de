defmodule MehrSchulferien.Repo.Migrations.Add2027WeekendPeriods do
  use Ecto.Migration
  ...
  def up do
    ...
    # Check if weekends for 2027 already exist
    existing_count = from(p in Period,
      where: p.location_id == ^deutschland.id,
      where: p.holiday_or_vacation_type_id == ^wochenende_type.id,
      where: fragment("EXTRACT(year FROM ?) = ?", p.starts_on, 2027)
    )
    |> Repo.aggregate(:count, :id)

    if existing_count > 0 do
      IO.puts("Skipping 2027 weekend creation - #{existing_count} weekends already exist")
    else
      # Generate all weekend periods for 2027
      weekend_periods_2027 = generate_weekend_periods(2027)

    # Insert weekend periods in batches
    Enum.chunk_every(weekend_periods_2027, 50)
    |> Enum.each(fn batch ->
      periods_data = Enum.map(batch, fn {starts_on, ends_on} ->
        %{
          starts_on: starts_on,
          ends_on: ends_on,
  ...
  def down do
    # Remove all weekend periods for 2027
    deutschland = Repo.one!(from l in Location, where: l.slug == "d")
    wochenende_type = Repo.one!(from h in HolidayOrVacationType, where: h.slug == "wochenende")
  ...
  defp generate_weekend_periods(year) do
    ...
    start_date
    |> Date.range(end_date)
    |> Enum.filter(&(Date.day_of_week(&1) == 6))  # Saturday = 6

...
query = from p in Period,
  where: p.is_school_vacation == true,
  where: fragment("? >= ? AND ? <= ?", p.starts_on, ^~D[2026-01-01], p.starts_on, ^~D[2026-12-31]),
...
query_2025 = from p in Period,
  where: p.is_school_vacation == true,
  where: fragment("? >= ? AND ? <= ?", p.starts_on, ^~D[2025-01-01], p.starts_on, ^~D[2025-12-31]),
  join: l in Location, on: p.location_id == l.id,

defmodule MehrSchulferienWeb.LayoutNavigationTest do
  ...
  describe "navigation functionality" do
    ...
    test "renders navigation template with proper structure", %{navigation_assigns: navigation_assigns} do
    ...
    test "renders mobile navigation sections", %{navigation_assigns: navigation_assigns} do
      ...
      assert Regex.scan(~r/<summary[^>]*>.*?Schulferien 2025.*?<\/summary>/s, html) |> length() == 1
      assert Regex.scan(~r/<summary[^>]*>.*?Schulferien 2026.*?<\/summary>/s, html) |> length() == 1
      assert Regex.scan(~r/<summary[^>]*>.*?Brückentage 2025.*?<\/summary>/s, html) |> length() == 1
      assert Regex.scan(~r/<summary[^>]*>.*?Brückentage 2026.*?<\/summary>/s, html) |> length() == 1
    end

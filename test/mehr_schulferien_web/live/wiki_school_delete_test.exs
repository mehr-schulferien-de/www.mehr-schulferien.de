defmodule MehrSchulferienWeb.WikiSchoolDeleteTest do
  ...
  describe "school deletion" do
    ...
    test "opens delete confirmation modal when delete button is clicked", %{conn: conn, school: school} do
    ...
    test "shows ZIP code hint when school has address", %{conn: conn, school: school, address: address} do
      {:ok, view, _html} = live(conn, ~p"/wiki/schools/#{school.slug}/edit")
    ...
    test "successfully deletes school with correct ZIP code", %{conn: conn, school: school, address: address, city: city} do
      {:ok, view, _html} = live(conn, ~p"/wiki/schools/#{school.slug}/edit")
    ...
    test "email contains all school and address data", %{conn: conn, school: school, address: address} do
      {:ok, view, _html} = live(conn, ~p"/wiki/schools/#{school.slug}/edit")

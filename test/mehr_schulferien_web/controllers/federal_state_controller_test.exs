defmodule MehrSchulferienWeb.FederalStateControllerTest do
  ...
  describe "index" do
    test "lists all federal_states", %{conn: conn} do
      conn = get(conn, Routes.federal_state_path(conn, :index))
  ...
  describe "new federal_state" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.federal_state_path(conn, :new))
  ...
  describe "create federal_state" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.federal_state_path(conn, :create), federal_state: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.federal_state_path(conn, :show, id)

      conn = get(conn, Routes.federal_state_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Federal state"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.federal_state_path(conn, :create), federal_state: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Federal state"
    end
  end

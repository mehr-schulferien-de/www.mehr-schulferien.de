defmodule MehrSchulferien.Calendars.VacationTypesTest do
  ...
  setup do
    # Create test data
    {:ok, country} = Locations.create_location(%{
      name: "Deutschland",
      slug: "deutschland",
      is_country: true
    })

    {:ok, federal_state} = Locations.create_location(%{
      name: "Bayern",
      slug: "bayern",
    ...
    {:ok, summer_vacation} = Calendars.create_holiday_or_vacation_type(%{
      name: "Sommerferien",
      slug: "sommerferien",
      colloquial: "Sommer",
    ...
    {:ok, winter_vacation} = Calendars.create_holiday_or_vacation_type(%{
      name: "Winterferien",
      slug: "winterferien",
      colloquial: "Winter",
      country_location_id: country.id,
    ...
    {:ok, public_holiday} = Calendars.create_holiday_or_vacation_type(%{
      name: "Tag der Arbeit",
      slug: "tag-der-arbeit",
      ...
      default_is_listed_below_month: true,
      default_is_school_vacation: false,  # Not a school vacation
  ...
  describe "list_for_federal_state/2" do
    test "returns vacation types used in the date range", %{
      ...
      {:ok, _summer_period} = Periods.create_period(%{
      ...
      {:ok, _winter_period} = Periods.create_period(%{
        location_id: federal_state.id,
    ...
    test "excludes periods shorter than 3 days", %{
      ...
      {:ok, _short_period} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: summer_vacation.id,
        starts_on: Date.add(today, 10),
        ends_on: Date.add(today, 11),  # Only 2 days
        is_school_vacation: true,
        is_public_holiday: false,
        is_valid_for_students: true,
        is_valid_for_everybody: false,
    ...
    test "excludes periods outside the date range", %{
      ...
      {:ok, _old_period} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: summer_vacation.id,
    ...
    test "excludes non-school vacation types", %{
      ...
      {:ok, _holiday_period} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: public_holiday.id,
        starts_on: Date.add(today, 10),
        ends_on: Date.add(today, 10),  # Single day
    ...
    test "returns unique vacation types even with multiple periods", %{
      ...
      {:ok, _period1} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: summer_vacation.id,
        starts_on: Date.add(today, 30),
        ends_on: Date.add(today, 40),
      ...
      {:ok, _period2} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: summer_vacation.id,
  ...
  describe "exists_for_state?/2" do
    test "returns true when vacation type exists for state", %{
      ...
      {:ok, _period} = Periods.create_period(%{
        location_id: federal_state.id,
        holiday_or_vacation_type_id: summer_vacation.id,
    ...
    test "returns false for different state", %{
      ...
      {:ok, other_state} = Locations.create_location(%{
        name: "Berlin",
        slug: "berlin",
        is_federal_state: true,
        parent_location_id: country.id
      ...
      {:ok, first_state} = Locations.create_location(%{
        name: "Hamburg",
        slug: "hamburg",
        is_federal_state: true,
        parent_location_id: country.id
